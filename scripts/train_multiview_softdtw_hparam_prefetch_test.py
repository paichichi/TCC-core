...
@dataclass
class CleanAugBatchPlan:
  clean_views: list[ViewRef]
  clean_group_indices: list[int]
  clean_camera_ids: list[int]
  aug_views: list[ViewRef]
  aug_seeds: list[int]
  aug_group_indices: list[int]
  aug_camera_ids: list[int]
  num_groups: int
...
def validate_method_configuration(args: argparse.Namespace) -> None:
  ...
  if args.view_mode == "single":
    ...
    if args.prefetch_batches not in (0, 1):
      raise ValueError("single view mode supports --prefetch-batches=0 or 1.")
...
def load_image(
  ...
  with Image.open(root / rel_path) as image:
    ...
    return transform_rgb(rgb, transform, transform_seed)


def load_rgb_image(root: Path, rel_path: str) -> Image.Image:
  with Image.open(root / rel_path) as image:
...
def plan_clean_aug_batch(
  ...
  for sequence in sequences:
    ...
    for group in sequence:
      ...
      group_idx += 1
  return CleanAugBatchPlan(
      clean_views=clean_views,
      clean_group_indices=clean_group_indices,
      clean_camera_ids=clean_camera_ids,
      aug_views=aug_views,
      aug_seeds=aug_seeds,
      aug_group_indices=aug_group_indices,
      aug_camera_ids=aug_camera_ids,
      num_groups=group_idx,
  )


def materialize_clean_aug_batch(
    root: Path,
    plan: CleanAugBatchPlan,
    clean_transform,
    aug_transform,
    device,
    pin_memory: bool = False,
  ...
  clean_images = [
      load_image(root, view.rel_path, clean_transform)
      for view in plan.clean_views
  ]
  aug_images = [
      load_image(
          root,
      ...
      for view, aug_seed in zip(plan.aug_views, plan.aug_seeds)
  ]
  return (
      move_batch_tensor(torch.stack(clean_images, dim=0), device, pin_memory),
      torch.tensor(plan.clean_group_indices, dtype=torch.long, device=device),
      torch.tensor(plan.clean_camera_ids, dtype=torch.long, device=device),
      move_batch_tensor(torch.stack(aug_images, dim=0), device, pin_memory),
      torch.tensor(plan.aug_group_indices, dtype=torch.long, device=device),
      torch.tensor(plan.aug_camera_ids, dtype=torch.long, device=device),
      plan.num_groups,
  )


def prefetch_clean_aug_sources(
    root: Path,
    plan: CleanAugBatchPlan,
    clean_transform,
    pin_memory: bool = False,
  ...
  aug_rgb_images = [
      load_rgb_image(root, view.rel_path) for view in plan.aug_views
  ]
  clean_batch = torch.stack(clean_images, dim=0)
  if pin_memory:
    clean_batch = clean_batch.pin_memory()
  return (
      clean_batch,
      torch.tensor(plan.clean_group_indices, dtype=torch.long),
      torch.tensor(plan.clean_camera_ids, dtype=torch.long),
      aug_rgb_images,
      plan.aug_seeds,
      torch.tensor(plan.aug_group_indices, dtype=torch.long),
      torch.tensor(plan.aug_camera_ids, dtype=torch.long),
      plan.num_groups,
  )
...
def prepare_clean_aug_batch_images(
  ...
  return materialize_clean_aug_batch(
      root,
      plan,
      clean_transform,
      aug_transform,
...
def main() -> None:
  ...
  if resume_path is not None:
    ...
    required_resume_keys = {"step", "model", "optimizer", "args"}
    missing_resume_keys = required_resume_keys - set(resume_checkpoint)
    ...
    mismatches = {
        key: (checkpoint_args.get(key), getattr(args, key, None))
    ...
    if "sample_rng_state" in resume_checkpoint:
  ...
  if use_prefetch and args.fusion_mode != "attention_pool":
    raise ValueError(
        "Batch prefetch currently requires attention_pool.")
  ...
  prefetch_rng_state_after_plan = None

  def submit_prefetch(rng_state):
    local_rng = random.Random()
    local_rng.setstate(rng_state)
    sequences = sample_paired_sequences(
        paired_tracks,
        episode_ids,
        task_to_episodes,
        args.batch_episode_pairs,
    ...
    return future, local_rng.getstate()
  ...
  if use_prefetch:
    ...
    prefetch_future, prefetch_rng_state_after_plan = submit_prefetch(
        rng.getstate())

  csv_name = "losses.csv" if is_main else f"losses_rank{rank}.csv"
  csv_path = args.out_dir / csv_name
  csv_mode = "a" if resume_path is not None else "w"
  if resume_path is not None:
    if not csv_path.is_file():
      raise FileNotFoundError(
          f"Resume requires an existing losses CSV: {csv_path}")
    with csv_path.open("r", newline="", encoding="utf-8") as existing_file:
      existing_rows = list(csv.DictReader(existing_file))
    if not existing_rows:
      raise ValueError(f"Resume losses CSV has no data rows: {csv_path}")
    csv_last_step = int(existing_rows[-1]["step"])
    if csv_last_step != start_step:
      raise ValueError(
          f"Resume losses CSV ends at step {csv_last_step}, but checkpoint "
          f"is step {start_step}.")
  with csv_path.open(csv_mode, newline="", encoding="utf-8") as f:
    ...
    for step in range(start_step + 1, args.max_iters + 1):
      ...
      if use_prefetch:
        ...
        if prefetch_rng_state_after_plan is None:
          raise RuntimeError("Missing prefetched RNG state.")
        prefetched = prefetch_future.result()
        rng.setstate(prefetch_rng_state_after_plan)
        if args.pixel_aug:
          (
              images,
              group_idx,
          ...
          aug_images = torch.stack([
              transform_rgb(rgb, aug_transform, aug_seed)
              for rgb, aug_seed in zip(aug_rgb_images, aug_seeds)
          ], dim=0)
          aug_images = move_batch_tensor(
              aug_images, device, args.pin_memory)
          aug_group_idx = aug_group_idx.to(
              device, non_blocking=args.pin_memory)
          aug_camera_ids = aug_camera_ids.to(
              device, non_blocking=args.pin_memory)
        else:
          images, group_idx, camera_ids, num_groups = prefetched
          aug_images = None
          aug_group_idx = None
          aug_camera_ids = None
        ...
        camera_ids = camera_ids.to(device, non_blocking=args.pin_memory)
      else:

...
STAMP=${STAMP:-method3_default_$(date +%Y%m%d_%H%M%S)}
BASE="${TCC_ROOT}/wsl_result/downstream_rvt2_lite_runs"
TCC_LOG_DIR="${TCC_ROOT}/wsl_result/tcc_core_logs"
RUN_ROOT="${BASE}/wsl_method3_default_${STAMP}"
TRAIN_LOG_DIR="${BASE}/wsl_method3_default_train_logs_${STAMP}"
EVAL_LOG_DIR="${BASE}/wsl_method3_default_eval_logs_${STAMP}"
RAW="${BASE}/wsl_method3_default_${STAMP}.csv"
SUMMARY="${BASE}/wsl_method3_default_summary_${STAMP}.csv"
OVERALL="${BASE}/wsl_method3_default_overall_${STAMP}.csv"
LEVELS="${BASE}/wsl_method3_default_level_summary_${STAMP}.csv"

TCC_RUNS=(
  "wsl_method3_vit_ln_8ts4v:${TCC_ROOT}/configs/wsl_lite_method3_soft_alignment_8ts4v.yaml:${TCC_ROOT}/wsl_result/tcc_core_runs/wsl_lite_method3_softalign_8ts4v_b4_i3000/checkpoint_003000.pt"
  "wsl_method3_vit_ln_8ts3v:${TCC_ROOT}/configs/wsl_lite_method3_soft_alignment_8ts3v.yaml:${TCC_ROOT}/wsl_result/tcc_core_runs/wsl_lite_method3_softalign_8ts3v_b4_i3000/checkpoint_003000.pt"
)

RVT_RUNS=(
  "wsl_method3_vit_ln_8ts4v:${TCC_ROOT}/downstream/rvt2_lite/configs/wsl_method3_vit_ln_8ts4v.yaml:${TCC_ROOT}/wsl_result/tcc_core_runs/wsl_lite_method3_softalign_8ts4v_b4_i3000/checkpoint_003000.pt"
  "wsl_method3_vit_ln_8ts3v:${TCC_ROOT}/downstream/rvt2_lite/configs/wsl_method3_vit_ln_8ts3v.yaml:${TCC_ROOT}/wsl_result/tcc_core_runs/wsl_lite_method3_softalign_8ts3v_b4_i3000/checkpoint_003000.pt"
)
...
mkdir -p "${TCC_LOG_DIR}" "${RUN_ROOT}" "${TRAIN_LOG_DIR}" "${EVAL_LOG_DIR}"
...
for item in "${RVT_RUNS[@]}"; do
  IFS=: read -r run _cfg _ckpt <<< "${item}"
...
for rep in $(seq 1 "${REPEATS}"); do
  for item in "${RVT_RUNS[@]}"; do
    IFS=: read -r run _cfg _ckpt <<< "${item}"
    log_name="wsl_method3_default_${STAMP}_rep${rep}"

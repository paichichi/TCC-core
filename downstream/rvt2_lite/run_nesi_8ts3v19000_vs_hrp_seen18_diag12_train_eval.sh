...
RUN_ROOT="${BASE}/nesi_8ts3v19000_vs_hrp_${STAMP}"
TRAIN_LOG_DIR="${BASE}/nesi_8ts3v19000_vs_hrp_train_logs_${STAMP}"
EVAL_LOG_DIR="${BASE}/nesi_8ts3v19000_vs_hrp_eval_logs_${STAMP}"
RAW="${BASE}/nesi_8ts3v19000_vs_hrp_${STAMP}.csv"
SUMMARY="${BASE}/nesi_8ts3v19000_vs_hrp_summary_${STAMP}.csv"
OVERALL="${BASE}/nesi_8ts3v19000_vs_hrp_overall_${STAMP}.csv"
LEVELS="${BASE}/nesi_8ts3v19000_vs_hrp_level_summary_${STAMP}.csv"
...
RUNS=(
  "nesi_vit_ln_8ts3v_19000:${TCC_ROOT}/downstream/rvt2_lite/configs/nesi_vit_ln_8ts3v_19000.yaml"
  "hrp_imagenet:${TCC_ROOT}/downstream/rvt2_lite/configs/hrp_imagenet.yaml"
)
...
for item in "${RUNS[@]}"; do
  IFS=: read -r run cfg <<< "${item}"
  test -s "${cfg}"
  pretrain=$(awk -F': ' '$1=="pretrain" {print $2}' "${cfg}")
  test -s "${pretrain}"
  echo "CHECK ${run}"
  echo "  cfg=${cfg}"
...
for rep in $(seq 1 "${REPEATS}"); do
  for item in "${RUNS[@]}"; do
    IFS=: read -r run _cfg <<< "${item}"
    log_name="nesi_8ts3v19000_vs_hrp_${STAMP}_rep${rep}"

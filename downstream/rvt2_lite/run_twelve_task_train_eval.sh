...
for run in "${RUNS[@]}"; do
  ckpt="${BASE}/twelve_runs/${run}/model_0.pth"
  test -s "${ckpt}"
  ls -lh "${ckpt}"
done
...
for rep in 1 2 3; do
  for run in "${RUNS[@]}"; do
    ...
    if "${CONDA_EXE}" run --no-capture-output -n "${CONDA_ENV}" python -m rvt.eval \
      --model-folder "${BASE}/twelve_runs/${run}" \
      ...
      --no-env-shutdown \
      > "${outlog}" 2>&1; then
      csv="${BASE}/twelve_runs/${run}/eval/${log_name}/model_0/eval_results.csv"

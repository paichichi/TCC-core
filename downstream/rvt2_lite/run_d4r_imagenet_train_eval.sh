...
STAMP=${STAMP:-d4r_imagenet_$(date +%Y%m%d_%H%M%S)}
BASE="${TCC_ROOT}/wsl_result/downstream_rvt2_lite_runs"
RUN_ROOT="${BASE}/d4r_imagenet_${STAMP}"
TRAIN_LOG_DIR="${BASE}/d4r_imagenet_train_logs_${STAMP}"
EVAL_LOG_DIR="${BASE}/d4r_imagenet_eval_logs_${STAMP}"
RAW="${BASE}/d4r_imagenet_${STAMP}.csv"
SUMMARY="${BASE}/d4r_imagenet_summary_${STAMP}.csv"
OVERALL="${BASE}/d4r_imagenet_overall_${STAMP}.csv"
LEVELS="${BASE}/d4r_imagenet_level_summary_${STAMP}.csv"

RUN=d4r_imagenet
CFG="${TCC_ROOT}/downstream/rvt2_lite/configs/d4r.yaml"
PRETRAIN="/home/paichichi/data/pretrain/D4R_IN_1M.pth"
...
for rep in $(seq 1 "${REPEATS}"); do
  log_name="d4r_imagenet_${STAMP}_rep${rep}"

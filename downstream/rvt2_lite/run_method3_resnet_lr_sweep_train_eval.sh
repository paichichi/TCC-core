...
STAMP=${STAMP:-method3_resnet_lr_sweep_$(date +%Y%m%d_%H%M%S)}
...
RUN_ROOT="${BASE}/method3_resnet_lr_sweep_${STAMP}"
TRAIN_LOG_DIR="${BASE}/method3_resnet_lr_sweep_train_logs_${STAMP}"
EVAL_LOG_DIR="${BASE}/method3_resnet_lr_sweep_eval_logs_${STAMP}"
RAW="${BASE}/method3_resnet_lr_sweep_${STAMP}.csv"
SUMMARY="${BASE}/method3_resnet_lr_sweep_summary_${STAMP}.csv"
OVERALL="${BASE}/method3_resnet_lr_sweep_overall_${STAMP}.csv"
LEVELS="${BASE}/method3_resnet_lr_sweep_level_summary_${STAMP}.csv"
TABLE="${BASE}/method3_resnet_lr_sweep_table_${STAMP}.csv"
...
RUN_SPECS=(
  "method3_linux_resnet_asym_v2_8ts4v_lr5e5:${TCC_ROOT}/configs/wsl_lite_method3_resnet_ln_8ts4v.yaml:0.00005"
  "method3_linux_resnet_asym_v2_8ts3v_lr5e5:${TCC_ROOT}/configs/wsl_lite_method3_resnet_ln_8ts3v.yaml:0.00005"
  "method3_linux_resnet_asym_v2_8ts4v_lr2p5e5:${TCC_ROOT}/configs/wsl_lite_method3_resnet_ln_8ts4v.yaml:0.000025"
  "method3_linux_resnet_asym_v2_8ts3v_lr2p5e5:${TCC_ROOT}/configs/wsl_lite_method3_resnet_ln_8ts3v.yaml:0.000025"
)
...
cd "${TCC_ROOT}"
RVT_RUNS=()
for item in "${RUN_SPECS[@]}"; do
  IFS=: read -r run cfg lr <<< "${item}"
  test -s "${cfg}"
  ckpt="${TCC_ROOT}/wsl_result/tcc_core_runs/${run}_b4_i3000/checkpoint_003000.pt"
  converted="${PRETRAIN_DIR}/${run}_b4_i3000_rvt2_resnet_pretrain.pt"

  if [ -s "${ckpt}" ]; then
    echo "TCC ${run} SKIP existing ${ckpt} $(date '+%F %T')"
  else
    echo "TCC ${run} START lr=${lr} $(date '+%F %T')"
    CUDA_VISIBLE_DEVICES="${DEVICE}" "${TCC_PYTHON}" -u train.py \
      --exp_cfg_path "${cfg}" \
      --device 0 \
      -- \
      --run-name "${run}_b4_i3000" \
      --lr "${lr}" \
      --max-iters 3000 \
      --save-every 3000 \
      --log-every 50 \
  ...
  if [[ "${run}" == *"8ts4v"* ]]; then
    rvt_cfg="${TCC_ROOT}/downstream/rvt2_lite/configs/method3_wsl_resnet_ln_8ts4v.yaml"
  else
    rvt_cfg="${TCC_ROOT}/downstream/rvt2_lite/configs/method3_wsl_resnet_ln_8ts3v.yaml"
  fi
  RVT_RUNS+=("${run}:${rvt_cfg}:${converted}")
done
...
for rep in $(seq 1 "${REPEATS}"); do
  for item in "${RVT_RUNS[@]}"; do
    ...
    log_name="method3_resnet_lr_sweep_${STAMP}_rep${rep}"

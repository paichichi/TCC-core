...
STAMP=${STAMP:-method2_8ts3v_min2_$(date +%Y%m%d_%H%M%S)}
BASE="${TCC_ROOT}/wsl_result/downstream_rvt2_lite_runs"
TCC_LOG_DIR="${TCC_ROOT}/wsl_result/tcc_core_logs"
RUN_ROOT="${BASE}/method2_8ts3v_min2_${STAMP}"
TRAIN_LOG_DIR="${BASE}/method2_8ts3v_min2_train_logs_${STAMP}"
EVAL_LOG_DIR="${BASE}/method2_8ts3v_min2_eval_logs_${STAMP}"
RAW="${BASE}/method2_8ts3v_min2_${STAMP}.csv"
SUMMARY="${BASE}/method2_8ts3v_min2_summary_${STAMP}.csv"
OVERALL="${BASE}/method2_8ts3v_min2_overall_${STAMP}.csv"
LEVELS="${BASE}/method2_8ts3v_min2_level_summary_${STAMP}.csv"

RUN=wsl_vit_ln_8ts3v_min2
TCC_CFG="${TCC_ROOT}/configs/wsl_lite_vit_ln_8ts3v_min2.yaml"
RVT_CFG="${TCC_ROOT}/downstream/rvt2_lite/configs/wsl_tcc_vit_ln_8ts3v_min2.yaml"
PRETRAIN="${TCC_ROOT}/wsl_result/tcc_core_runs/wsl_lite_vit_ln_8ts3v_min2_b4_i3000/checkpoint_003000.pt"
...
echo "LEVELS=${LEVELS}"
echo "TCC_CFG=${TCC_CFG}"
echo "RVT_CFG=${RVT_CFG}"
echo "PRETRAIN=${PRETRAIN}"

cd "${TCC_ROOT}"
test -s "${TCC_CFG}"
if [ -s "${PRETRAIN}" ]; then
  echo "TCC ${RUN} SKIP existing ${PRETRAIN} $(date '+%F %T')"
else
  echo "TCC ${RUN} START $(date '+%F %T')"
  CUDA_VISIBLE_DEVICES="${DEVICE}" "${TCC_PYTHON}" -u train.py \
    --exp_cfg_path "${TCC_CFG}" \
    --device 0 \
    > "${TCC_LOG_DIR}/${RUN}_${STAMP}.log" 2>&1
  echo "TCC ${RUN} DONE $(date '+%F %T')"
  tail -50 "${TCC_LOG_DIR}/${RUN}_${STAMP}.log"
fi
...
for rep in $(seq 1 "${REPEATS}"); do
  log_name="method2_8ts3v_min2_${STAMP}_rep${rep}"
